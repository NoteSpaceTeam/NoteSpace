 ...
 * @param handler
 */
export function createSetBlockApply(style: BlockStyle, handler: ApplyBlockStyleHandler) {
 ...
 * @param handler
 */
export function createSetInlineApply(style: InlineStyle, triggerLength: number, handler: ApplyInlineStyleHandler) {
  return (editor: Editor, range: Range) => {
    const key = style;
    ...
    handler(key, selection, true, triggerLength);
  };
}
