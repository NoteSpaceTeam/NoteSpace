...
function useWaypointFugue(){
  ...
  const createBetween = (a: string | undefined, b: string | undefined): string => {
    ...
    const leafSender = a.slice(
      secondLastComma - replicaId.length,
      secondLastComma
    );
    if (leafSender === replicaId) {
      ...
      if (lastValueIndices[leafCounter] === leafValueIndex) {
        ...
        setLastValueIndices(prev =>  [
          ...prev.slice(0, leafCounter),
          valueIndex,
          ...prev.slice(leafCounter + 1)
        ]);
    ...
    const insertLocal = (char: string, cursor: number): string => {
      ...
      const sortedElements = [...elements, character]
        .sort((a, b) => getTagId(a).localeCompare(getTagId(b)));
    ...
    const sortTree = (arr: string[]): string[] => {
      ...
      const sorted = array.sort((a, b) => getTagId(a).localeCompare(getTagId(b)));
      console.log(sorted);
      return sorted
    }
