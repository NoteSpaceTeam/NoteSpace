...
describe('Insert line', () => {
  const insertLine = () => {
    ...
    const batch = toBatch(splitNode({}, [0, 0], 0), splitNode({ type: BlockStyles.p }, [0], 1));
  ...
  test('Should undo paste', () => {
    ...
    expect(operation.cursor).toEqual(pointToCursor(editor, { path: [editorOperation.path[0] + 1, 0], offset: 0 }));
  });
